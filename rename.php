...
?><!DOCTYPE html>
<html lang="de-DE" ng-app="renameApp" ng-controller="main as c">
	<head>
		<meta charset="UTF-8" />
		<title>Rename</title>
		<meta name="viewport" content="width=device-width, initial-scale=1, user-scalable=yes">
		<?php echo TmRename::echoStyle() ?>
	</head>

	<body>
		<div id="pageWrap" >
			<h1>Rename</h1>
			<?php if(!empty($GLOBALS['TM_RENAME_SETUP']['infoMessage'])) : ?>
				<?php echo $GLOBALS['TM_RENAME_SETUP']['infoMessage'] ?>
			<?php endif; ?>
			<p>
				<code><?php echo $GLOBALS['TM_RENAME_SETUP']['old'] ?></code> --&gt; <code><?php echo $GLOBALS['TM_RENAME_SETUP']['new'] ?></code>
			</p>

			<div style="display: flex; justify-content: space-between;">
				<button type="button" class="btn btn-primary" ng-click="c.doStart()" ng-if="c.output.length===0">start</button>
				<button type="button" class="btn btn-primary" ng-click="c.doStart()" ng-if="c.output.length>0">once more</button>

				<a class="btn btn-primary" href="/">open current domain</a>
			</div>
		...
		<script type="text/ng-template" id="/main.html">
			<section class="mainOutput">
				<section ng-repeat="i in c.output">
					<header><h1>{{i.title}}</h1></header>
					<p class="sqlQuery">
					...
					<p class="summary" ng-if="i.summary">
						...
						<span class="s_col" ng-if="i.summery.col">{{i.summary.col}}</span>
						<span class="s_count" ng-if="i.summery.count">{{i.summary.count}} Umbenennungen</span>
						<span class="s_text" ng-if="i.summary.text">{{i.summary.text}}</span>
					</p>
				</section>
				<hr />
				<button type="button" class="btn btn-primary" ng-click="c.doStart()" ng-if="c.output.length>0">once more</button>
...
class TmRename {
	...
	public static function echoStyle() {
		...
		/*!
		 * Generated using the Bootstrap Customizer (http://getbootstrap.com/customize/?id=ee8b1c69b278f6d181b5aa73c346b3bf)
		 * Config saved to config.json and https://gist.github.com/ee8b1c69b278f6d181b5aa73c346b3bf
		 *//*!
		...
		#pageWrap { margin:2em auto; max-width: 1000px; border:1px solid #ddd; background: #fff; padding:2em; }
		h1 { margin:0; }
		...
		section.mainOutput p.summary span { padding:5px; display: inline-block; background: #f0f0f0; border-radius: 3px; }
		section.mainOutput p.summary span.s_count { font-weight: bold; }
		section.mainOutput p.summary span.s_text { display: block; margin:1em 0; }
		</style>
		<?php
	}
}
